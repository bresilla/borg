...
async fn run_command(matches: ArgMatches) -> Result<(), Box<dyn std::error::Error>> {
    let mut command = "ros2 param dump".to_owned();

    let node_name = matches.get_one::<String>("node_name").unwrap();
    command.push_str(" ");
    command.push_str(&node_name.to_string());

    if matches.get_one::<String>("output_dir") != None {
        let output_dir_value = matches.get_one::<String>("output_dir").unwrap();
        command.push_str(" --output-dir ");
        command.push_str(&output_dir_value.to_string());
    }
